...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        long numbOfTasks = project.getTaskLists().stream()
                    .filter(inProgressTasks::contains)
                    .mapToLong(tl -> tl.getTasks().size())
                    .sum();

        long timeItTakes = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(d -> ChronoUnit.DAYS.between(d.atStartOfDay(),LocalDate.now().atStartOfDay()))
                .reduce(0L,(sum, current) -> sum = sum + current);
        //Then
        assertEquals(10,timeItTakes/numbOfTasks);
    }
